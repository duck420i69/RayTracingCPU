...
bool Object::hit(const Ray& ray, Packet& result) const {
			...
			if (bvh[iter].next_index == 0) {
				...
				if (tri.hit(ray, result)) {
					if (material == nullptr) closest_triangle = &tri;
						...
						texel texcord = getTexcord(tri, { result.u, result.v });
						texcord.x = texcord.x * material->width - 0.5f;
						texcord.y = texcord.y * material->height - 0.5f;
						color = color * material->texture_map[static_cast<int>(texcord.x) + static_cast<int>(texcord.y) * material->width];
						if (color.w > 0.99f) {
	...
	if (closest_triangle != nullptr) {
		...
		else if (material->texture_map.size() == 0) {
			result.color = closest_triangle->col;
			result.material = material;
		}
		else {
			texel texcord = getTexcord(*closest_triangle, { result.u, result.v });
			texcord.x = texcord.x * material->width - 0.5f;
			texcord.y = texcord.y * material->height - 0.5f;
			result.color = material->texture_map[static_cast<int>(texcord.x) + static_cast<int>(texcord.y) * material->width] * (1 - color.w) + color * color.w;
			result.material = material;
		}
